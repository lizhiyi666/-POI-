...
def run_Statistical(dataset, experiment_comments):
    ...
    # 收集偏序矩阵（如果存在）
    po_matrix = None
    if hasattr(test_data, 'sequences') and len(test_data['sequences']) > 0:
        if 'po_matrix' in test_data['sequences'][0]:
            po_matrix = [seq['po_matrix'] for seq in test_data['sequences']]
...
def evaluation(task, dataset, cuda, results_log, experiment_comments, generated_only, init_exposed_num, exp_num, max_weeks):
    if task != "Stat":
        ...
        with open (results_log, "a+") as f:
            ovr_str = f", OVR: {JSD_Values['OVR']}" if 'OVR' in JSD_Values else ""
            f.writelines(f"Distance: {JSD_Values['Distance']}, Radius: {JSD_Values['Radius']}, Interval: {JSD_Values['Interval']}, DailyLoc: {JSD_Values['DailyLoc']}, Category: {JSD_Values['Category']}, 'G-RANK': {JSD_Values['G-RANK']}{ovr_str}\n")
