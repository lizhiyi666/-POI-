...
class Sequence:
    def __init__(
        ...
        po_encoding: np.ndarray | TensorType = None,
        po_matrix: np.ndarray | TensorType = None,
    ) -> None:
        super().__init__()
...
class Batch:
    def __init__(
        ...
        po_encoding: Union[TensorType, None] = None,  # 新增参数
        po_matrix: Union[TensorType, None] = None,
    ):
        super().__init__()
        self.time = time
        self.condition1 = condition1
    ...
    def from_sequence_list(sequences: List[Sequence]) -> "Batch":
        ...
        if sequences[0].po_encoding is not None:
            po_encoding = torch.stack([sequence.po_encoding for sequence in sequences])
        else:
            po_encoding = None

        # 处理po_matrix
        if sequences[0].po_matrix is not None:
            po_matrix = torch.stack([sequence.po_matrix for sequence in sequences])
        else:
            po_matrix = None
    ...
    def remove_unnescessary_padding(
            ...
            kept: object,
            tmax: object,
            po_encoding: Union[TensorType, None] = None,
            po_matrix: Union[TensorType, None] = None,
    ) -> object:
        """
    ...
    def thin(self, alpha: TensorType[float]) -> Tuple["Batch", "Batch"]:
        ...
        # po_encoding是全局编码，不需要应用序列级别的mask
        ...
        return self.remove_unnescessary_padding(
            ...
            po_encoding= po_encoding_removed ,
            po_matrix=self.po_matrix,
            condition1_indicator=self.condition1_indicator,
            condition2_indicator=self.condition2_indicator,
            condition3_indicator=self.condition3_indicator,
