...
class MixtureIntensity(nn.Module):
    ...
    def __init__(
        ...
        self.mlp = nn.Sequential(
            nn.Linear(3*embedding_size+ po_encoding_dim, embedding_size),
            nn.ReLU(),
    ...
    def get_intensity_parameters(
        ...
        po_emb: TensorType[float, "batch", "embedding"] = None,
        ...
        # 构建输入向量，包含po_encoding
        input_list = [seq_emb, dif_time_emb, seq_cond_emb]
        if po_emb is not None:
            input_list.append(po_emb)
        parameters = self.mlp(torch.cat(input_list, dim=-1))
        return torch.split(
            parameters,
